...
function checkInstallScript(&$errorMsg, $value = null)
{
    ...
    if (!$result) {
        $errorMsg = xtr('X-Cart 5 installation script not found. Restore it  and try again');
    }
        ...
        if ($version) {
            ...
            if (version_compare($version, constant('LC_MYSQL_VERSION_MIN')) < 0) {
                ...
                if (!\Includes\Utils\Database::isInnoDBSupported()) {
                    $result = false;
                    $errorMsg = xtr('MySQL server doesn\'t support InnoDB engine. It is required for X-Cart 5 operation');
                }
            }
...
function doFinishInstallation(&$params, $silentMode = false)
{
    ...
    @mail($params["login"], "X-Cart 5 installation complete", $message,
        "From: \"X-Cart 5 software\" <" . $params["login"] . ">\r\n" .
        "X-Mailer: PHP");
...
<div class="field-label"><?php echo xtr('X-Cart 5 software has been successfully installed and is now available at the following URLs:'); ?></div>
<img src="http://www.x-cart.com/img/spacer2.gif" width="1" height="1" alt="" />
...
function change_config(&$params)
{
    ...
    // Check clean ulrs availability
    $isCleanUrlsAvailable = inst_http_request_clean_urls();
    ...
    foreach ($config as $num => $line) {
        ...
        $replacements = array(
            ...
            'table_prefix = "' . $_params['mysqlprefix'] . '"',
        ...
        if ($setCleanURL && preg_match('/^enabled.*=.*/', $line)) {
            // Actualize clean URL availability
            $line = preg_replace('/^enabled.*=.*/', 'enabled = ' . ($isCleanUrlsAvailable ? 'on' : 'off'), $line);
            $setCleanURL = false;
        }
        ...
        if (preg_match('/^\[clean_urls\]/', $line)) {
            // Prepare to change next line (clean URL availability)
            $setCleanURL = true;
        }
    }
...
function inst_http_request($url_request, $returnCode = false)
{
    ...
    return $returnCode ? $result->code : $response;
}
...
function make_check_report($requirements)
{
    ...
    $report = array();
    $report[] = 'X-Cart 5 version ' . constant('LC_VERSION');
 ...
 * @return void
 */
function fatal_error_extended($txt) {
...
function update_config_settings($params)
{
    ...
    // Update arrival date of sample products
...
function module_cfg_install_db(&$params)
{
    ...
    $paramFields = array(
        ...
        'xlite_web_dir'    => array(
            'title'       => xtr('X-Cart 5 web directory'),
            'description' => xtr('Path to X-Cart 5 files within the web space of your web server (E.g.: /shop).'),
        ...
        // Check if database settings provided are valid
            ...
            if ($connection) {
                ...
                if (!checkMysqlVersion($mysqlVersionErr, $currentMysqlVersion, true)) {
                    fatal_error_extended($mysqlVersionErr . (!empty($currentMysqlVersion) ? '<br />(current version is ' . $currentMysqlVersion . ')' : ''));
                    $checkError = true;
                }
                    ...
                    if (is_array($res)) {
                        ...
                        foreach ($res as $row) {
                            if (in_array(get_db_tables_prefix() . 'products', $row)) {
                                warning_error(xtr('Installation Wizard has detected X-Cart 5 tables'));
                                $checkWarning = true;
                                break;
                            }
                        }
                ...
                fatal_error_extended(xtr('Can\'t connect to MySQL server specified:pdoerr<br /> Press \'BACK\' button and review MySQL server settings you provided.', array(':pdoerr' => (!empty($pdoErrorMsg) ? ': ' . $pdoErrorMsg : ''))));
                $checkError = true;
            }
        }
...
/**
 * Output Javascript handler: module_cfg_create_admin_js_back
 *
 * @return string
 */
function module_cfg_create_admin_js_back()
{
    global $params;
...
<div id="cache-rebuild-failed" class="cache-error" style="display: none;"><span><?php echo xtr('Oops! Cache rebuild failed.'); ?></span> <?php echo xtr('Check for possible reasons <a href="http://kb.x-cart.com/display/XDD/Setting+time+limit+of+your+server">here</a>.'); ?></div>
...
?>
    function step_next() {
        ...
        if (document.ifrm.elements['params[confirm_password]'].value == "") {
            document.ifrm.elements['params[confirm_password]'].className += ' error';
            document.ifrm.elements['params[confirm_password]'].focus();
