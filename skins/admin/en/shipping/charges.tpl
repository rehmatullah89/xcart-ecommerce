...
<p align="justify">
  {t(#SHIPPING = flat + ITEMS*per_item + WEIGHT*per_weight + SUBTOTAL*(% of subtotal)/100;#,_ARRAY_(#X#^config.Units.weight_unit))}
</p>
<br />
<p align="justify">
...
<p align="justify">
  {t(#SHIPPING = raw on-line_rate + flat + ITEMS*per_item + WEIGHT*per_weight + SUBTOTAL*(% of subtotal)/100;#,_ARRAY_(#X#^config.Units.weight_unit))}
</p>
<br />
<p align="justify">
...
<br />
<p align="justify">
<b>{t(#Flat shipping charge per X ordered (per Y)#,_ARRAY_(#X#^config.Units.weight_symbol,#Y#^config.Units.weight_unit))}</b><br />
    ...
    <tr>
      <td>
        <select name="methodid" onchange="document.charges_methods.submit();">
          ...
          <option FOREACH="getShippingMethods(),m" value="{m.getMethodId()}" selected="{isSelected(m,#method_id#,methodid)}">{m.getName():stripTags}</option>
        </select>
      </td>
