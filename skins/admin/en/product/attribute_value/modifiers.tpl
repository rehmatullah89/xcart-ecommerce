...
<div class="modifiers" IF="{isModified(attributeValue)}">
  ...
  <div class="popup">
    ...
    {end:}
    <div class="default clearfix"><label><input type="checkbox" name="{getName(#default#,id)}"{if:isDefailt(attributeValue)} checked{end:} data-title="{t(#Default#)}" /> {t(#Default option#)}</label></div>
    <div class="arrow"></div>
  </div>
</div>
