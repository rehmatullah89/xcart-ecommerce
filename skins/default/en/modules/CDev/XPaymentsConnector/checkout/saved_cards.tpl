...
<div class="saved-cards-box xpc-box"{if:!checkCheckoutAction()} style="display: none;"{end:}>
  ...
  <ul class="saved-cards single">

    {foreach:profile.getSavedCards(),cc}
      <li class="all-items{if:!cc.is_default} hidden{end:}">
        <input type="radio" name="payment[saved_card_id]" value="{cc.card_id}"{if:cc.is_default} checked="checked"{end:} class="all-items hidden" id="saved_card_{cc.card_id}" />
  ...
  <p class="switch-cards-link">
    <a href="javascript: void(0);" onclick="javascript: jQuery('.cart-items').get(0).loadable.switchCards();" >{t(#Show all saved credit cards#)}</a>
  </p>
