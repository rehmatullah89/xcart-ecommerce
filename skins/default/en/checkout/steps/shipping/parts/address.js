...
CheckoutAddressView.prototype.assignHandlers = function(event, state)
{
  if (state.isSuccess) {
    ...
    this.base.find('button.address-book').each(
      function() {
        var elm = jQuery(this);
        if (elm.attr('onclick')) {
    ...
    if (form) {
      ...
      form.getElements()
        .keyup(_.bind(this.handleElementKeypress, this))
      ...
      this.base.find('button.address-book').click(_.bind(this.openAddressBook, this));
...
CheckoutAddressView.prototype.handleFormPreValidateSubmit = function(event, widget)
{
  if (
    ...
    var createProfile = widget.form.getElements().filter('#create_profile').get(0);
...
{
  var createProfile = event.target.form.getElements().filter('#create_profile').get(0);
  if (createProfile) {
    createProfile.commonController.unmarkAsInvalid();
  }
...
CheckoutAddressView.prototype.openAddressBook = function(event, elm)
{
  ...
  popup.load(elm.get(0), 'address-book');
...
CheckoutAddressView.prototype.saveForm = function(saveState)
{
  ...
  if ((form.commonController.isChanged() || this.isNeedRecheck()) && !form.isBgSubmitting) {
    if (!onlyEmail && !onlyGuestAgree && form.validate(true)) {
      ...
      form.elements.namedItem('same_address')
      && form.elements.namedItem('same_address')[1]
      && form.elements.namedItem('same_address')[1].commonController.isChanged()
      && form.elements.namedItem('same_address')[1].commonController.validate(true)
    ) {
