...
class Checkout extends \XLite\Controller\Customer\Cart
{
    ...
    protected function doPayment()
    {
        ...
        if (\XLite\Model\Payment\Transaction::PROLONGATION == $result) {
            ...
            if ($cart->isPayed()) {
                $status = $paymentStatus ?: $cart->getPaymentStatus();
    ...
    protected function doActionReturn()
    {
        ...
        $declinedStatuses = array(
            ...
            \XLite\Model\Order::STATUS_CANCELED,
        );
        ...
        if (!$cart) {
            ...
            0 < $cart->getOpenTotal()
            && !in_array($cart->getStatus(), $declinedStatuses)
        ) {
            $url = $this->buildURL(
                'cart',
                'add_order',
            ...
            if ($cart->isPayed()) {
                ...
                if (!empty($transactions)) {
                    ...
                    if ($lastTransaction->isCanceled()) {
                        $status = \XLite\Model\Order::STATUS_CANCELED;
                    }
                }
            }
     ...
     */
    protected function getStatusTarget($status)
    {
        switch ($status) {
            case \XLite\Model\Order::STATUS_CANCELED:
                $result = 'checkoutCanceled';
                break;

            case \XLite\Model\Order::STATUS_FAILED:
    ...
    {
        $this->processCartProfile($doCloneProfile);
    ...
    protected function doActionUpdateProfile()
    {
        ...
        if (
            ...
            && !$this->requestData['billingAddress']
