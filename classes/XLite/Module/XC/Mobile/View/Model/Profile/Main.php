...
namespace XLite\Module\XC\Mobile\View\Model\Profile;
...
class Main extends \XLite\View\Model\Profile\Main implements \XLite\Base\IDecorator
{
    ...
    protected function getFormFieldsForSectionMain()
    {
        if (\XLite\Core\Request::isMobileDevice()) {
            foreach ($this->mainSchema as $name => $field) {
                $this->mainSchema[$name][static::SCHEMA_FIELD_ONLY] = true;
                $this->mainSchema[$name][static::SCHEMA_PLACEHOLDER] = $this->mainSchema[$name][static::SCHEMA_LABEL];
            }
        }
