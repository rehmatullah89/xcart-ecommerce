...
class Cart extends \XLite\Controller\Customer\Cart implements \XLite\Base\IDecorator
{
    ...
    protected function getItemDescription(\XLite\Model\OrderItem $item)
    {
        ...
        if ($item->getVariant()) {
            ...
            foreach ($item->getProduct()->getVariantsAttributes() as $a) {
                $variantsAttributes[$a->getId()] = $a->getId();
            }

            foreach ($item->getAttributeValues() as $v) {
                $av = $v->getAttributeValue();
                if ($av->getAttribute()->isVariable($item->getProduct())) {
        ...
        return $result ?: parent::getItemDescription($item);
    }
    ...
    protected function getProductAvailableAmount(\XLite\Model\OrderItem $item)
    {
        return $item->getVariant() && !$item->getVariant()->getDefaultAmount()
            ...
            : parent::getProductAvailableAmount($item);
    }
