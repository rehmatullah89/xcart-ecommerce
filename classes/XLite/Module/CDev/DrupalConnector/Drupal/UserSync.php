...
class UserSync extends \XLite\Module\CDev\DrupalConnector\Drupal\ADrupal
{
    ...
    /**
     * List of non-linked accounts (X-Cart accounts)
     * 
     * @var array
     */
    ...
    public function addUserSyncForm(array &$form)
    {
        if ($this->isUserSynchronizationRequired()) {
            ...
            $formDescription = <<<OUT
<p>Non-synchronized user accounts found in the Drupal and X-Cart databases. This means that when user is logged in to the Drupal site but doesn't have an X-Cart account, the user cannot use the catalog of products as a registered user. Clicking on the button below will link the Drupal and X-Cart accounts, observing the following rules:</p>
<ul>
<li>If non-linked accounts with same email are present in both Drupal and X-Cart databases, these accounts will be linked.</li>
<li>If an account is present in Drupal but is missing in the X-Cart database, the linked account will be created in the X-Cart database with a randomly generated password and the same email as in the Drupal account.</li>
<li>If an account is present in X-Cart but is missing in the Drupal database, the linked account will be created in the Drupal database with a randomly generated password and the same email as in the X-Cart account.</li>
</ul>
...
OUT;
    ...
    /**
     * Gather all accounts from Drupal and X-Cart databases and store them into internal properties
    ...
    protected function findAllAccounts()
    {
        ...
        if (is_null($this->lcAccounts)) {
            // Get array of X-Cart accounts with order_id = NULL
    ...
    protected function prepareNonSynchronizedAccounts()
    {
        if (
            ...
            if ($this->findAllAccounts()) {
                ...
                // Accounts presented in X-Cart but missed in Drupal
                $this->accountsMissedInDrupal = array();

                // Accounts presented in X-Cart and Drupal but not linked via cms_profile_id field
                $this->nonLinkedAccounts = array();

                // Find accounts presented in X-Cart but missed in Drupal
    ...
    /**
     * Link X-Cart accounts with Drupal accounts
     * 
     * @return void
     */
    ...
    /**
     * Get valid Drupal user name from login (email) of X-Cart account 
     * 
     * @param string $email X-Cart login field (email)
     *  
     * @return string
     */
