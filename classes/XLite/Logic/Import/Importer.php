...
class Importer extends \XLite\Base
{
    ...
    public function __construct(array $options = array())
    {
        $this->options = array(
            ...
            'clearItems'   => isset($options['clearItems']) ? $options['clearItems'] : false,
            'clearImportDir' => isset($options['clearImportDir']) ? $options['clearImportDir'] : false,
            'dir'          => isset($options['dir']) ? $options['dir'] : LC_DIR_VAR . static::IMPORT_DIR,
        ...
        static::$languageCode = isset($options['languageCode'])
            ...
            : \Includes\Utils\ConfigParser::getOptions(array('language', 'default'));
   ...
   /**
    ...
    {
        \XLite\Core\Database::getRepo('XLite\Model\TmpVar')->setVar('importCancelFlag', false);
    ...
    {
        \XLite\Core\Database::getRepo('XLite\Model\TmpVar')->setVar('importCancelFlag', true);
        \XLite\Core\Database::getRepo('XLite\Model\TmpVar')->removeEventState('import');
    }
    ...
    protected function initialize()
    {
        ...
        foreach ($this->getOptions()->files as $path) {
            if (\XLite\Core\Archive::getInstance()->isArchive($path)) {
                \XLite\Core\Archive::getInstance()->unpack($path, $this->getOptions()->dir);
