...
abstract class AProcessor extends \XLite\Base implements \SeekableIterator, \Countable
{
    ...
    protected function getCategoryByPath($path)
    {
        ...
        if (!isset($this->categoriesCache[implode('/', $path)])) {
            $this->categoriesCache[implode('/', $path)] = \XLite\Core\Database::getRepo('XLite\Model\Category')->findOneByPath($path);
        }

        return $this->categoriesCache[implode('/', $path)];
    }
    ...
    protected function getFile()
    {
        if (
            !isset($this->file)
            || $this->file->eof()
            || $this->file->key() == $this->countCache[$this->file->getPathname()]
        ) {
            $path = null;
            $found = false;
    ...
    protected function getNextRow(\SplFileObject $file)
    {
        ...
        do {
            ...
            if ($this->isEmptyRow($row) && !$file->eof()) {
                $row = null;
                $file->next();
                $this->position++;
                $this->importer->getOptions()->position = $this->importer->getOptions()->position + 1;
    ...
    protected function importData(array $data)
    {
        ...
        if ($model) {
            ...
            if ($this->importer->getOptions()->clearItems) {
                $this->getRepository()->delete($model, false);
    ...
    protected function assembleModelConditions(array $data)
    {
        $conditions = array();
        foreach ($this->getKeyColumns() as $column) {
            if (!empty($data[$column[static::COLUMN_NAME]])) {
   ...
   /**
     ...
     */
    protected function normalizeValueAsAttributeGroup($value)
    {
        $result = null;
        $value = $this->getDefLangValue($value);

        if ($value) {
            $result = \XLite\Core\Database::getRepo('XLite\Model\AttributeGroup')->findOneByName($value);
            if (!$result) {
    ...
    /**
     * Check - column header is equal with p[attern or not
     *
     * @param string  $pattern              Reg.exp. pattern
