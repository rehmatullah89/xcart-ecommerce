...
class Generator extends \XLite\Base implements \SeekableIterator, \Countable
{
    ...
    {
        \XLite\Core\Database::getRepo('XLite\Model\TmpVar')->setVar('exportCancelFlag', false);
        \XLite\Core\Database::getRepo('XLite\Model\TmpVar')->initializeEventState(
            'export',
            array('options' => $options)
        );
        \XLite\Core\EventTask::export();
    }
    ...
    {
        \XLite\Core\Database::getRepo('XLite\Model\TmpVar')->setVar('exportCancelFlag', true);
        \XLite\Core\Database::getRepo('XLite\Model\TmpVar')->removeEventState('export');
    }
    ...
    public function getTickDuration()
    {
        ...
        if ($this->getOptions()->time && 1 < $this->getOptions()->position) {
            ...
            $tick = \XLite\Core\Database::getRepo('XLite\Model\TmpVar')->getVar('exportTickDuration');
            if ($tick) {
                $result = $tick;
            }
        }
    ...
    protected function defineStep()
    {
        ...
        if (!\XLite\Core\Database::getRepo('XLite\Model\TmpVar')->getVar('exportCancelFlag')) {
