...
abstract class AView extends \XLite\Core\Handler
{
    ...
    protected function registerResources(array $resources, $index, $interface = null)
    {
        foreach ($resources as $type => $files) {
            $method = $this->assembleRegisterResourcesTypeName($type);
    ...
    protected function printTagAttributes(array $attributes)
    {
        ...
        foreach ($attributes as $name => $value) {
            ...
            $pairs[] = func_htmlspecialchars($name) . '="' . func_htmlspecialchars(trim($value)) . '"';
        }
    ...
    {
        $parts = array_slice(explode('\\', get_called_class()), 1);
