...
class Minicart extends \XLite\View\SideBarBox
{
    ...
    {
        $cart = $this->getCart();
        return $cart->isMaxOrderAmountError()
            ? $this->getMaxOrderAmountErrorReason()
            : ($cart->isMinOrderAmountError() ? $this->getMinOrderAmountErrorReason() : '');
    }
    ...
    {
        $attributes = array(
            'id'    => 'lc-minicart-' . $this->getParam(static::PARAM_DISPLAY_MODE),
            'class' => array('lc-minicart', 'lc-minicart-' . $this->getParam(static::PARAM_DISPLAY_MODE), 'collapsed'),
        );
