...
class Module extends \XLite\Model\Repo\ARepo
{
    ...
    {
        $moduleInfo = $this->findOneBy(array('author' => $author, 'name' => $module, 'fromMarketplace' => true));
        $page = 0;
        if ($moduleInfo) {
            $sessionCellName = \XLite\View\ItemsList\Module\Install::getSessionCellName();
            $data = \XLite\Core\Session::getInstance()->$sessionCellName;

            $qb = $this
                ->createPureQueryBuilder('m')
                ->select('m.moduleID')
                ->where('m.fromMarketplace = :true');

            if ($data && isset($data['sortBy'])) {
                $qb = $qb->orderBy($data['sortBy'], $data['sortOrder']);
            }
    ...
    {
        $moduleInfo = $this->findOneBy(array('author' => $author, 'name' => $module, 'fromMarketplace' => 0));
        $page = 0;
        if ($moduleInfo) {
            ...
            $qb = $this
                ->createPureQueryBuilder('m')
                ->select('m.moduleID')
                ->where('m.fromMarketplace = :false')
                ->setParameter('false', 0);

            $sessionCellName = \XLite\View\ItemsList\Module\Manage::getSessionCellName();
            $data = \XLite\Core\Session::getInstance()->$sessionCellName;
            if (!empty($data['sortBy']) && !empty($data['sortOrder'])) {
                $qb->orderBy($data['sortBy'], $data['sortOrder']);
            }
