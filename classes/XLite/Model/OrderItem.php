...
class OrderItem extends \XLite\Model\Base\SurchargeOwner
{
        ...
        foreach ($this->getAttributeValues() as $attributeValue) {
            if ($attributeValue->getAttributeValue()) {
                $result .= '||'
                    ...
                    . '::'
                    . $attributeValue->getAttributeValue()->asString();
            }
        }
    ...
    public function isValid()
    {
        ...
        if (
            ...
            && (
                ...
                || $this->getProduct()->hasMultipleAttributes()
            )
        ) {
            $result = array_keys($this->getAttributeValuesIds()) == $this->getProduct()->getMultipleAttributesIds();
        }
    ...
    public function setAttributeValues(array $attributeValues)
    {
        ...
        foreach ($attributeValues as $av) {
            ...
            $newValue->setValue($av->asString());
            $newValue->setOrderItem($this);
            $this->addAttributeValues($newValue);
            $method = 'setAttributeValue' . $av->getAttribute()->getType();
            if (method_exists($newValue, $method)) {
                $newValue->{$method}($av);
            }
        }
    }
