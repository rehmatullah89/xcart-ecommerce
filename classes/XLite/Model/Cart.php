...
class Cart extends \XLite\Model\Order
{
            ...
            if ($auth->isLogged()) {
                $cart = \XLite\Core\Database::getRepo('XLite\Model\Cart')->findOneBy(
                    array(
                        ...
                        'status'  => \XLite\Model\Cart::STATUS_TEMPORARY
                    )
                );
    ...
    /**
     * Defines the status for payed cart
     *
     * @return string Status of cart
     */
    public function getPaymentStatus()
    {
        ...
        if (0 < $this->getOpenTotal()) {
            $paymentStatus = \XLite\Model\Order::STATUS_QUEUED;
    ...
    {
        // Merge carts
        $carts = $this->getRepository()->findBy(
            array(
                'profile' => $profile,
                'status'  => static::STATUS_TEMPORARY
            )
        );
